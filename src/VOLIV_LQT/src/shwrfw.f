...
c-----
        ...
        integer n
        real dt, rayp, gaussalp, thp,svs,delay, invwgt
...
c-----
     ...
     1      status='unknown')
        ...
        do 1000 i=1,nurftn
            ...
            read(3,*)n, dt, rayp, gaussalp,thp,svs, delay, invwgt
            ls = lgstr(fname)
        write(LOT,3)i, kstnm, gaussalp,thp,svs,invwgt, fname(1:ls)

 1000   continue
 9000   continue
        write(LOT,2)
        close(3)
    1   format('NUM ','Station ',' Gauss','HP-Period','SVS', ' Rftn-Wt',
     1      ' File-Name')
    2   format('----------------------------------------'   ,
     1  '----------------------------------------'   )
    3   format(i3,1x,a8,f10.2,f10.2,f10.2, f10.2,1x,a)
...
c-----
        ...
        integer n(MXRF)
        real dt(MXRF), rayp(MXRF), gaussalp(MXRF), thp(MXRF),svs(MXRF),
...
c-----
     ...
     1          nzmin(j), nzmon(j), nzday(j)
            read(3,*)n(j), dt(j), rayp(j), gaussalp(j), thp(j),svs(j),
     1                  delay(j), invwgt(j)
            murftn = murftn + 1
 1000   continue
 9000   continue
...
c-----
     ...
     2          nzmin(j), nzmon(j), nzday(j)
            write(3,'(i5,f8.3,f8.3,f8.3,f8.3,f8.3,f8.3,f8.3)')
     1          n(j), dt(j), rayp(j), gaussalp(j),thp(j),svs(j),
     2          delay(j), invwgt(j)
 2000   continue
        close(3)
        return

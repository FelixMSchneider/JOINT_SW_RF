...
c
c
c       OKT 2021 changes by Felix Schneider in order to produce L-,Q-,T- RFs
c       DEC 2021 changes by Felix Schneider apply a butterworth highpass filter to the Q-RF data
c       since this is done in the preprocessing of the data                      
c
c            thp (period of hp-filter) has to defined by user5 header variable in data sac file
c            svs has to defined by user6 header variable in data sac file
c
c-----
        implicit none
        ...
        logical dop, dotwo
        real delay, dt, rayp, gaussalp,thp,svs,invwgt
        integer n, iout
        character mname*80
        character kstnm*8
...
c-----
            call gcmdln(dop,dotwo,delay,dt,rayp,gaussalp,thp,svs,n,
     1          mname,iout)
            twnmin = -abs(delay)
...
c-----
            ...
            call doit(' ', n, dt, rayp, gaussalp,thp,svs, delay,1.0,0,
...
c-----
            call doit(' ', n, dt, rayp, gaussalp,thp,svs,delay,1.0,1,
     1          twnmin,twnmax,dop,dotwo,mname,
     2              iout,outbin,kstnm, nzyear, nzjday, 
     3              nzhour, nzmin, nzmon, nzday)
        else
...
c-----
 ...
 1000       continue
                ...
                read(1,*)n, dt, rayp, gaussalp, thp,svs, delay, invwgt
                ls = lgstr(fname)

                call doit(fname, n, dt, rayp, gaussalp,thp,svs, delay,
     1              invwgt,
...
c-----
        ...
        subroutine doit(fname, n,dt,rayp,gaussalp, thp,svs,delay,invwgt,
     ...
     3              nzhour, nzmin, nzmon, nzday)
        ...
        character fname*(*)
        character kstnm*8
        real rayp, delay, dt, gaussalp,twmin, twmax, invwgt, thp, svs
...
c       thp     R   High-Pass filter parameter (period of butterworth)
c       svs     R   surface vs for rotation to LQT system
...
c-----
        ...
        real df, dfac, fac, wc, hpbutt, fac2
        integer nerr
...
c-----
        ...
        do 4000 i=1,n21
            ...
            Zin = cmplx(1.0,0.0)
c
c          FS_comment: HERE THE RECEIVER FUNCTIONS ARE CALCULATED
c
c      
c            svs is now defined by user6 header variable in data sac file
c            svs = 0 <=> ZRT rotation

            call excit(freq,dop,rayp,Q,Zin,iout,invdep,mmx,svs)
c
c           compute L-RF (L/L) directly as spike at 0
c            
            do kl=1,mmax+1
               L(kl) =  dcmplx(1.0d+00, 0.0d+00)
            enddo

c
c       include the desired time shift
c
            do 4100 kl=1,mmax+1
...
c-----
  ...
  425       continue 
c-----
c           now Gaussian filter and HP Butterworth Filters
c-----
c           define HP corner frequency 
c           if thp <=0: wc=0  <=> hpbutt=1 <=> no hpfilter
...
c           apply HP-Butterworth to Q-RF data
c-----
                if(i.eq.1)then
...
c-----
c       Now qdata(1,1)      --- qdata(nd2,1) is perturbed 1st layer
c       Now qdata(1,2)      --- qdata(nd2,2) is perturbed 2nd layer
c       Now qdata(1,mmax)   --- qdata(nd2,mmax) is perturbed halfspace
c       Now qdata(1,mmax+1) --- qdata(nd2,mmax+1) is unperturbed RFTN
c-----

c----  normation: find maximum of ldata
...
c----- norm Q-RF traces by maximum of L
  ...
  429      continue
        ...
        call copyts(qdata(1,mmax+1),x,n)
        if(outbin)then
...
c-----
        ...
        subroutine gcmdln(dop,dotwo,delay,dt,rayp,gaussalp,thp,svs,
     1      nsamp,mname,iout)
c-----
c       parse command line arguments
...
c       dokjar  L   - 
c       iout    I   - output 1 = rftn, 2 = z, 3 = r time series, 4 = Q-RF
c-----
        logical dop, dotwo
        real delay, dt, rayp, gaussalp, thp,svs
        integer nsamp, iout
        character mname*(*)
...
C       mname = ' '
   ...
   11   i = i + 1
            ...
            else if(name(1:4).eq.'-THP')then
                i = i + 1
                call mgtarg(i,name)
                read(name,'(bn,f10.0)')thp
            else if(name(1:4).eq.'-SVS')then
                i = i + 1
                call mgtarg(i,name)
                read(name,'(bn,f10.0)')svs
...
c-----
     ...
     1      ' -ALP alpha -DT dt -NSAMP nsamp -THP thp -SVS svs'
        write(LER,*)
     ...
     1  '-M   model   (default none )    Earth model name'
        write(LER,*)
     1  '-THP thp     (default 30.0  )   High pass filter parameter'
        write(LER,*)
     1  '-SVS svs     (default 3.0  )    Surface Vs for rotation to LQT'
        write(LER,*)
...
c           use fixed s-velocity for rotation (vs=3.0)
c-----

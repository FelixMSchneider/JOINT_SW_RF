...
c
c
c            call excit(freq,dop,rayp,Z,Zin,iout,invdep,mmx)

            call excit(freq,dop,rayp,Z,Zin,iout,invdep,mmx)
            call excit(freq,dop,rayp,ZR,Zin,5,invdep,mmx)
...
c
            ...
            if(i.eq.1)then
                ...
                pldata(1,kl) = real(ZR(kl))
            else if(i.eq.n21)then
                pdata(2,kl) = real(Z(kl))
                pldata(2,kl) = real(ZR(kl))
            else
                j = 2*i -1
                k = j + 1
                pdata(j,kl) =  real(Z(kl))
                pdata(k,kl) = aimag(Z(kl))
                pldata(j,kl) =  real(ZR(kl))
                pldata(k,kl) = aimag(ZR(kl))
            endif
 4100       continue
 4000   continue
c-----
...
c-----
            ...
            call realft(pldata(1,kl),eata,nd2,isign,dt)
c-----
...
c----  normation: find maximum of ldata
           ...
           norm=0.0
           do 427 i = 1,n
                if(pldata(i,mmax+1)>norm) then
                   norm=pldata(i,mmax+1)
                endif
  427       continue
...
c----- norm Q-RF traces by maximum of L
           ...
           do 429 kl=1,mmax+1
           do 430 i = 1,n
              pldata(i,kl)=pldata(i,kl)/norm
              pdata(i,kl)=pdata(i,kl)/norm
  430      continue
  429      continue

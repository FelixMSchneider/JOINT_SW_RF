...
c        open(4,file='tmpsrfi.03',form='formatted',access='sequential')
        rewind 4
c-----
...
c-----
 ...
 1000   continue
        ...
        call getfhv('USER5   ',thp,nerr)
        call getfhv('USER6   ',svs,nerr)
        ...
        if(thp.eq. -12345.)then
        write(*,*) "WARNING: USER5 not defined, set HP-filter to 30s "
        thp = 30.0
        endif

        if(svs.eq. -12345.)then
        write(*,*) "WARNING: USER6 not defined, set svs to 3.0 km/s "
        write(*,*) "(svs is the surface vs, used for rotation to LQT) "
        svs = 3.0
        endif
